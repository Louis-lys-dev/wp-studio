...
还是在后台点着建字段组，模板里还是 `get_field()` / `get_sub_field()`。acf-json 只是在「保存」这个动作之后多写一个文件。

变的只有三件事：

| | 接 acf-json 之前 | 之后 |
|---|---|---|
| 上线 | 导出 JSON → 线上导入 | 推代码就有了 |
| 字段改动历史 | 查不到 | `git log acf-json/` |
| 多人同时改 | 谁后导入谁覆盖，无提示 | Git 冲突（能看见，但机器生成的 JSON 手动 merge 很难受） |

#### 接上之后第一件事：重新保存所有老字段组

**已经存在于数据库里的字段组不会自动生成 json**——ACF 只在保存时写文件。

把每个字段组打开、点一次「更新」（什么都不用改），文件才会出现。漏掉的那些还是只活在数据库里，等于没接。

```bash
git status --short acf-json/
# 每个字段组应该对应一个 group_xxxxxxxx.json
```
...
以前生产环境的数据库各改各的，你在线上后台调个字段它就一直在。现在 JSON 也进了部署流程，**在线上后台改字段会同时改掉那台机器的 json，而下次 `git pull` 会覆盖它**。

所以：**字段在本地/开发环境改 → commit → 部署**。线上后台只看不改。

客户或同事会动线上字段的话，这条要提前说清楚，否则会出现「我明明改了，发个版就回去了」。

#### Sync available 是什么

其他环境拉了新代码后，「自定义字段」列表里会出现一个 **Sync available** 标签。

**不点它字段也是生效的**——JSON 里的字段组在运行时直接可用，前台后台都能正常编辑内容。点 Sync 只是把它导进那台机器的数据库，让这个字段组本身在那边变成可编辑的。生产环境通常不需要点。

#### 减少冲突

多人协作时 Git 冲突发生在一大坨机器生成的 JSON 里，手动 merge 很痛苦。两个办法：

- **一个字段组一个 owner**
- **按模块拆细** —— 模块化主题本来就该一个 module 一个字段组，拆细之后两个人碰同一个文件的概率接近零

#### 其他

- 文件名是字段组的 **key** 不是标题（`group_65f3a1b2c4d5e.json`），改标题不会改文件名，看不出是哪个组只能打开看 `"title"`
- 删字段组时新版 ACF 会一起删掉 json；**删完 `git status` 确认一下**，留下的孤儿文件会在别人拉代码时让这个组「复活」
- ACF 免费版和 Pro 都有 Local JSON
